...
function main() {
    ...
    const notificationService = new NotificationService(notificationRepository, cloudMessageService, cloudStorageService, emailService)
    const jwtUtil = new JWTUtil()
    const userRepo = new UserRepository()
    const hashUtil = new HashUtil()
    const authService = new AuthService(userRepo, jwtUtil, hashUtil, notificationService)
